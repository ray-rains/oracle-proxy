...
function weightedPick(used: Set<number>, sentiment: number): DrawnCard {
...
function buildSystemPrompt(input: string, cards: DrawnCard[]): string {
  const lower = input.toLowerCase()
...
function buildUserMessage(input: string, cards: DrawnCard[]): string {
  const lines = cards
...
function loadStored(): StoredReading | null {
  try {
...
function saveStored(reading: StoredReading): void {
...
type Phase =
  ...
  | "hovering"     // Overlay slowly darkening
  | "completing"   // Overlay racing to black
  | "oracle"       // Oracle input UI visible
  | "loading"      // API call in flight
  | "reading"      // Cards + narrative visible
  | "recalled"     // Stored session reading being revealed

// ─── Card Images ─────────────────────────────────────────────────────────────

interface CardImageMap {
  [cardName: string]: { upright?: string; reversed?: string }
}
...
interface OraclePortalProps {
  cardImages?: string
}

export function OraclePortal({ cardImages = "{}" }: OraclePortalProps) {
  ...
  const [imageMap, setImageMap]           = useState<CardImageMap>({})
  ...
  // ── Card image resolver ────────────────────────────────────────────────────
  function getImageUrl(card: ArcanaCard, reversed: boolean): string | null {
...
function InputUI({ userInput, setUserInput, onSubmit, isLoading }: InputUIProps) {
...
interface ReadingUIProps {
  ...
  getImageUrl:   (card: ArcanaCard, reversed: boolean) => string | null
}
...
function CardTile({ drawn, visible, imageUrl }: CardTileProps) {
  const W = 160
  const H = 280
...
function NarrativeBlock({ text, visible }: { text: string; visible: boolean }) {
  if (!text) return null
  return (
    <div
      style={{
